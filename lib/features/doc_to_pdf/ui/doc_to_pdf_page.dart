...
import '../controller/doc_to_pdf_controller.dart';
import '../model/doc_to_pdf_model.dart';

/// DocToPdfPage 纯UI层，负责显示界面和处理用户交互
class DocToPdfPage extends StatefulWidget {
  const DocToPdfPage({super.key});

  @override
  State<DocToPdfPage> createState() => _DocToPdfPageState();
}

class _DocToPdfPageState extends State<DocToPdfPage> {
  /// 已选择的DOC文件路径
  String? _selectedDocFile;

  /// 转换状态信息
  String _statusMessage = '请选择DOC/DOCX文件开始转换';
  ...
  Future<void> _loadExportRecords() async {
    final records = await DocToPdfController.getExportRecords();
    if (mounted) {
      setState(() {
  ...
  Future<void> _onSelectFilePressed() async {
    final filePath = await DocToPdfController.selectDocFile();

    if (mounted) {
      setState(() {
        if (filePath != null && filePath.isNotEmpty) {
          _selectedDocFile = filePath;
  ...
  Future<void> _onConvertPressed() async {
    if (_selectedDocFile == null || _selectedDocFile!.isEmpty) {
      if (mounted) {
        setState(() {
          _statusMessage = '请先选择DOC文件';
        });
      }
      return;
    }
    ...
    // 获取文件名和输出文件名
    final fileName = _selectedDocFile!.split(Platform.pathSeparator).last;
    final fileNameWithoutExt = fileName.substring(0, fileName.lastIndexOf('.'));
    final timestamp = DateTime.now().millisecondsSinceEpoch;
    final pdfFileName = '${fileNameWithoutExt}_$timestamp.pdf';

    try {
      // 执行转换
      final result = await DocToPdfController.convertToPdf(
        docFilePath: _selectedDocFile!,
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          CupertinoButton.filled(
            onPressed:
                _isConverting || _selectedDocFile == null
                    ? null
                    : _onConvertPressed,
            child: _isConverting
                ? const Row(
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    _selectedDocFile!.split(Platform.pathSeparator).last,
                    style: TextStyle(
                      fontSize: 14,
