...
/// DocToPdfPage 纯 UI 层：DOC / DOCX 转 PDF。使用统一转换脚手架。
...
class _DocToPdfPageState extends State<DocToPdfPage> {
  ...
  Future<void> _onDeleteRecord(int timestamp) async {
    ...
    if (!ok) return;
    final success = await DocToPdfController.deleteExportRecord(timestamp);
    if (mounted && success) {
      setState(() =>
  ...
  Widget build(BuildContext context) {
    return ConversionScaffold(
      title: LocalizationEngine.text('tool_doc_pdf_title'),
    ...
    return ListView(
      ...
      children: [
        ConversionInfoCard(
          ...
          text: LocalizationEngine.text('conv_tip_doc'),
        ),
        const SizedBox(height: 16),
        ConversionPrimaryButton(
          label: LocalizationEngine.text('conv_select_doc'),
