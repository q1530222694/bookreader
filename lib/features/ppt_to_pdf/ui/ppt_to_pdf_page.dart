...
/// PptToPdfPage 纯 UI 层：PPT / PPTX 转 PDF。使用统一转换脚手架。
...
class _PptToPdfPageState extends State<PptToPdfPage> {
  ...
  Future<void> _onConvertPressed() async {
    ...
    try {
      final result = await PptToPdfController.convertToPdf(
        pptFilePath: _selectedFile!,
  ...
  Future<void> _onDeleteRecord(int timestamp) async {
    ...
    if (!ok) return;
    final success = await PptToPdfController.deleteExportRecord(timestamp);
    if (mounted && success) {
      setState(() =>
  ...
  Widget build(BuildContext context) {
    return ConversionScaffold(
      title: LocalizationEngine.text('tool_ppt_pdf_title'),
    ...
    return ListView(
      ...
      children: [
        ConversionInfoCard(
          ...
          text: LocalizationEngine.text('conv_tip_ppt'),
        ),
        const SizedBox(height: 16),
        ConversionPrimaryButton(
          label: LocalizationEngine.text('conv_select_ppt'),
